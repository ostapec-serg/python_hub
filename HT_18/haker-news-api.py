...
class CheckTopic:
    ...
    def main(self):
        ...
        if self.topic == 'askstories':
            ...
            raise ValueError\
                (f"Category {self.topic} don't exist! askstories,"
...
class Stories:
    ...
    def _create_list_id(self):
        ...
        for i_d in response.json()[0:5]:  # specify the number of ids here
            id_list.append(i_d)
        return id_list
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        autostart_category = CheckTopic(sys.argv[1])
        start = autostart_category.main()
        start.get_data()
    elif len(sys.argv) == 1:
        autostart_category = CheckTopic("")
        start = autostart_category.main()
        start.get_data()
    else:
        raise ValueError\
            (f"expected 1 values to unpack. give {len(sys.argv) - 1}")

...
class Person:
    ...
    # person balance
    def balance(self):
        user_balance = Bureaucracy().check_balance(self.name)
        balance = 0
    ...
    def change_pin(self):
        ...
        if new_pin1 == new_pin2:
            Bureaucracy().change_pin(self.name, new_pin2)
        else:
    ...
    def money_transfer(self):
        ...
        if (transfer_amount < my_balance) and \
            ...
            Bureaucracy().operation_succes(new_balance, self.name,
                                           transfer_amount)
            client_balance = self.balance()
            new_client_balance = client_balance + transfer_amount
            Bureaucracy().deposite_money(new_client_balance,
                                         name_to_transfer,
                                         transfer_amount)
            print(f"{self.name}, You have transferred from your account "
    ...
    def deposite(self):
        ...
        Bureaucracy().deposite_money(new_balance, self.name, depo)
        ...
        if money < my_balance and (my_balance + money) > 0:
            if ATM().atm_cash() >= money:
                ...
                if some_val is not False:
                    ...
                    Bureaucracy().operation_denied(self.name, money)
            else:
                print(f"ATM cannot dispense the selected amount\n"
            ...
            print(f"Not enough money in the account!\n"
                  f"in your account {self.balance()}")
            Bureaucracy().operation_denied(self.name, money)
                ...
                if 0 <= operation_num <= 6:
                    if operation_num == 1:
                        ...
                        print(f"{name} in your account "
                              f"{person.balance()} USD!\n")
                    elif operation_num == 2:
                        sleep(0.3)
                        raw = person.withdraw_cash()
                        if not raw:
                            pass
                        else:
